defmodule Bella.PeriodicTask do
  @moduledoc """
  ...
  @doc "Registers and starts a new task given `Bella.PeriodicTask` attributes"
  ...
  @doc "Registers and starts a new `Bella.PeriodicTask`"
  @spec register(t()) :: {:ok, pid} | {:error, term()}
  def register(%__MODULE__{id: id} = task) do
    Event.task_registered(%{}, %{id: id})
    DynamicSupervisor.start_child(__MODULE__, {Bella.PeriodicTask.Runner, task})
  end

  @doc "Unregisters and stops a `Bella.PeriodicTask`"
